...
<div style="display:flex;gap:6px;margin-bottom:20px;align-items:center">
  {"".join(
    ...
    for i,(v,l) in enumerate([(n,"HCPs both surveys"),(n_va,f"VA shown ({(round(n_va/n*100) if n>0 else 0)}%)"),(n_shift,f"Belief shifted ({round(n_shift/n*100)}%)"),(n_ltip,f"LTIP top-2 ({(round(n_ltip/n*100) if n>0 else 0)}%)"),(n_vora,f"Has Vora patients ({(round(n_vora/n*100) if n>0 else 0)}%)")])
  )}
</div>""", unsafe_allow_html=True)
    ...
    with c1:
        funnel=[
            ...
            (f"Access VA shown in visit",int(pd.to_numeric(df['access_va'],errors='coerce').fillna(0).sum())),
        ]
        ls,cs=zip(*funnel)
    ...
    with c1:
        if 'patient_demand' in df.columns:
            bins=pd.cut(df['patient_demand'].dropna(),bins=[0,.25,.5,.75,1.01],labels=['Low (0–0.25)','Mid-low','Mid-high','High (0.75–1.0)'])
