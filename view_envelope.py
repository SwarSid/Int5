...
<div style="margin-bottom:20px">
  ...
  <p style="font-size:12px;color:#475569;max-width:700px;line-height:1.65">
    ...
    for col,(lbl,val,sub,clr) in zip(mc,[
        ...
        ("HAS VORA PTS", f"{(round(n_vora/n_f*100) if n_f>0 else 0)}%",  f"n={n_vora}",GREEN),
        ("LTIP TOP-2",   f"{(round(n_ltip/n_f*100) if n_f>0 else 0)}%",  f"n={n_ltip}",PURPLE),
        ("VA SHOWN",     f"{(round(n_va/n_f*100) if n_f>0 else 0)}%",     f"n={n_va}",  AMBER),
    ]):
        with col:
            st.markdown(f"""
...
<div style="background:white;border:1px solid {MGRAY};border-radius:12px;padding:16px 18px;margin-bottom:12px">
  <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px">
    ...
    ltip_top2_pct= (round(n_ltip/n_f*100) if n_f>0 else 0)
    if ltip_avg < 6:
        _gap(
            "LTIP GAP · PET C3_35Z",
