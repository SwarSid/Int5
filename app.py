...
def filter_bar(df, key_pfx="g"):
    ...
    label = f"Showing **{n_f}** of {n_t} HCPs"
    if sel_spec != "All": label += f" · {sel_spec}"
    if sel_sett != "All": label += f" · {sel_sett}"
    if sel_tt   != "All": label += f" · {sel_tt}"
    st.markdown(f'<div style="font-size:10px;color:{DGRAY};margin:2px 0 12px">{label}</div>',
